...
*** =XPropertyLog PropName=
    ...
    - Reads the X property named by =PropName= (a string) and displays its
      value. The [[../src/branch/master/etc/xmonadpropwrite.hs][etc/xmonadpropwrite.hs script]] in xmobar's distribution can be
...
*** =BufferedPipeReader Alias [(Timeout, Bool, "/path/to/pipe1"), ..]=
    ...
    - Take a look at [[../src/branch/master/etc/status.sh][etc/status.sh]]
